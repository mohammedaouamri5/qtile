...
class Keybindings:
    ...
    def create_groups_keys(self):
        ...
        move_next = Key(mod+modifier, NEXT_GROUP, Functions.next_group()) 
        move_prev = Key(mod+modifier, PREV_GROUP, Functions.prev_group()) 
        
        swap_next = Key(mod+swap_modifier, NEXT_GROUP, Functions.window_to_next_group()) 
        swap_prev = Key(mod+swap_modifier, PREV_GROUP, Functions.window_to_prev_group()) 

        move_next_screen = Key(mod , NEXT_GROUP, lazy.next_screen()) 
...
class Mouse:
    ...
    def init_mouse(self):
        mouse = [
            Drag([self.mod], "Button1", lazy.window.set_position(),
                 start=lazy.window.get_position()),
            Drag([self.mod], "Button3", lazy.window.set_size(),
